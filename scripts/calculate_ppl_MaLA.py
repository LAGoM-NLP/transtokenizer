...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained('MaLA-LM/mala-500-10b', padding_side="right")
    model = PeftModel.from_pretrained(base_model, 'MaLA-LM/mala-500-10b', torch_dtype=torch.bfloat16)
    ...
    def gen(dataset, tokenizer):
        for example in tqdm(dataset, dynamic_ncols=True):
            text = example['text']
            tokenized_text = tokenizer.encode(text, padding="max_length", return_tensors='pt', truncation=True, return_special_tokens_mask=True, max_length=768 )[0]
